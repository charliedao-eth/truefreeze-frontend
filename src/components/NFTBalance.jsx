...
function NFTBalance({ filterByContractAddress = "" }) {
    ...
    <div style={{ padding: "15px", maxWidth: "1030px", width: "100%" }}>
      <div style={styles.NFTs}>
        <Skeleton loading={!NFTBalances?.result}>
          {NFTBalances?.result &&
            NFTBalances.result.filter((nft) => !filterByContractAddress ? true : (nft.token_address.toLowerCase() === filterByContractAddress)).map((nft, index) => {
              // /\ filter results to a single contract. if no address is passed in, just show the whole result set
