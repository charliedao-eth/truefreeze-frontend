...
function DashboardData({ account }) { // TODO create generic helpers for fetching data to separate UI from logic
  ...
  const { runContractFunction: getFrTokenTotalSupply, error: getFrTokenTotalSupplyError } =
    useWeb3Contract({
      abi: rinkebyContracts.frToken.abi,
      contractAddress: rinkebyContracts.frToken.address,
      functionName: "totalSupply",
    });
  const { runContractFunction: getFrTokenAllowance, error: getFrTokenAllowanceError } =
    useWeb3Contract({
      abi: rinkebyContracts.frToken.abi,
      contractAddress: rinkebyContracts.frToken.address,
  ...
  const { runContractFunction: getFrTokenBalance, error: getFrTokenBalanceError } =
    useWeb3Contract({
      abi: rinkebyContracts.frToken.abi,
      contractAddress: rinkebyContracts.frToken.address,
  ...
  const { runContractFunction: getFrzTotalSupply, error: getFrzTotalSupplyError } =
    useWeb3Contract({
  ...
  const { runContractFunction: getFrzBalance, error: getFrzBalanceError } =
    ...
    useEffect(() => {
      (async () => {
        ...
        setFrTokenTotalSupply(frTokenTotalSupplyResults?.toString() || "Failed to load");
        setFrTokenAllowance(frTokenAllowanceResults?.toString() || "Failed to load");
    ...
    return (
      <Fragment>
        ...
        {getFrTokenTotalSupplyError && JSON.stringify(getFrTokenTotalSupplyError)} {/* TODO real error handling */}
