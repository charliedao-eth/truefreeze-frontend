...
export default function useToken({ contract }) {
    ...
    const { getFrTokenTotalSupply, frTokenTotalSupplyError } = _frTokenTotalSupply({ contract, account });
    const { getFrTokenAllowance, frTokenAllowanceError } = _frTokenAllowance({ contract, account });
    const { getFrTokenBalance, frTokenBalanceError } = _frTokenBalance({ contract, account });
    const { getFrzTotalSupply, frzTotalSupplyError } = _frzTotalSupply({ contract, account });
    const { getFrzAllowance, frzAllowanceError } = _frzAllowance({ contract, account });
    const { getFrzBalance, frzBalanceError } = _frzBalance({ contract, account });
    const { getWrappedTokenAllowance, wrappedTokenAllowanceError } = _wrappedTokenAllowance({ contract, account });
    ...
    useEffect(() => {
        ...
        async function asyncEffect() {
            ...
            const handleFetchError = (err) => { console.error(err); return null; };
            
            const fetches = fetchAndSetFns.map(([fetchFn,]) => { 
                return fetchFn().then((result) => result?.toString() || null) // trigger every fetch and convert the result to a string or give up and return null if the result is falsey
...
function sanitizeTokenError(error) { // converts the error to a string that can be embedded in the DOM
    if (error) {
