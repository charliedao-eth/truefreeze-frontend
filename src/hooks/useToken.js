...
export default function useToken({ contract }) {
    const { isInitialized: isMoralisInitialized, account, chainId } = useMoralis();
    ...
    const isWrappedTokenAllowed = async ({ spender }) => genericIsTokenAllowed({ spender, tokenAddress: getWrappedNative(chainId) });

    const genericTokenApproval = async ({ spender, tokenAddress, tokenABI, spenderParamName, amountParamName }) => {
        const options = {
            chain: chainId,
            address: tokenAddress,
    ...
    const allowWrappedToken = async ({spender}) => genericTokenApproval({ spender, tokenAddress: getWrappedNative(chainId), tokenABI: wrappedTokenMetadata.ABI, spenderParamName: wrappedTokenMetadata.spenderParamName, amountParamName: wrappedTokenMetadata.amountParamName });
...
const isTokenAllowed = (allowance) => {
    if (typeof allowance !== 'string' || allowance.length <= 0 || !allowance) { //if not a string, empty string, or falsey
